...
class PathJSONParser {
    ...
    fun parse2(jObject: JSONObject): List<List<HashMap<String, String>>> {
        val routes: MutableList<List<HashMap<String, String>>> = ArrayList()
        var jRoutes: JSONArray? = null
        var jLegs: JSONArray? = null
        var jSteps: JSONObject? = null
        try {
            jRoutes = jObject.getJSONArray("routes")
            /** Traversing all routes  */
            for (i in 0 until 1) {
                ...
                /** Traversing all legs  */
                for (j in 0 until 1) {
                    jSteps = (jLegs[j] as JSONObject).getJSONObject("distance")
                    var distence = jSteps.getString("text")
                    jSteps = (jLegs[j] as JSONObject).getJSONObject("duration")
                    var duration = jSteps.getString("text")
                }
            }
        } catch (e: JSONException) {
            e.printStackTrace()
