...
class ItemFoundViewModel(activity: Activity) : BaseObservable() {
    private val webServiceRepository: WebServiceRepository = WebServiceRepository(activity)
    lateinit var venuListModel: LiveData<Resource<BaseModel>>




    fun foundLostItem(storeType : HashMap<String,String>): LiveData<Resource<BaseModel>> {
        venuListModel = webServiceRepository.foundLostItem(storeType)
